...
const pokeTypes = pokémon.filter((poke) => poke.types.length > 1);
...
const pokeNamesGreater99 = pokémon
  .filter((poke) => poke.id > 99)
  .map((poke) => poke.name);

console.log(pokeNamesGreater99);

// an array with just the names of the pokémon whose only type is poison
const poisonPoke = pokémon
  .filter((poke) => poke.types.includes("poison"))
  .map((poke) => poke.name);

console.log(poisonPoke);

// an array containing just the first type of all the pokémon whose second type is "flying"
const secondTypes = pokémon
